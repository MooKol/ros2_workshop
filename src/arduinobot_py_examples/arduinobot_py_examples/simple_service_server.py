...
class SimpleServiceServer(Node):
    def __init__(self):
        super().__init__("simple_setvice_server")
...
def main():
        rclpy.init()
        simple_setvice_server = SimpleServiceServer()
        rclpy.spin(simple_setvice_server)  ## Keep the node active and the time and node running
        simple_setvice_server.destroy_node()
        rclpy.shutdown()
