...
public:
    SimpleServiceClient(int a, int b) :Node("simple_service_client")
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Service not availible ...");
            if (!rclcpp::ok())
            {
                RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Interrupt while waiting for services");
                return;
            }
        }
...
int main(int argc, char*argv[])
{
    ...
    if (argc !=3)
    {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Wrong number");
        return 1;
    }
